...
The purpose of this report is to utilize wearable device data to predict the activity of the wearer.  The training data contains sensed data from the human body and given 5 activity classes for his/her activities. The modeling method used here is random forest.  The model from the training data will be applied to the testing data to predict the activity class for each record. The accuracy for this model is 0.99 from this cross-validation result.  Final submission shows that 19 of 20 predictions are correct.
...
Addinitionally, I have removed several variables ("pitch_belt", etc.) which made the fit fail when I have been trying CART method.
...
The data will be divided into the training set (70%) and the test set (30%) for cross validation.

```{r, echo=TRUE, cache=TRUE}
set.seed(2020)
inTrain <- createDataPartition (selected$classe, p = 0.70, list = FALSE)
training <- selected[inTrain,]
testing <- selected[-inTrain,]
```

The target classes are 5 and it is appropriate to use a tree model.  Here I use the random forest method.

### Training

The training data is supplied to "train" method
...
Now the model can be appiled to the remaining test data to produce the predicted classes "predo0".  The predicted results will be evaluated if they match to the actual data using confusionMatrix.
...
Submission (after the deadline) shows that 19 of 20 predictions are correct.
...
The result here is a sample result from my succeeded trial of CART method.
```
CART 
