...
The purpose is to utilize wearable device data to predict the activity of the wearer.  The training data contains sensed data from the human body and given 5 activity class for his/her activity. The objective here is to predict the activity class for the testing data. The modeling method used is CART.  The accuracy for this model is 0.497.  Final submission shows that only 8 of 20 predictions are correct.
...
The training data contains sensed data from the human body and labelled his/her activity as "A" to "E" in the coulmn "classe".  The testing data has the same format in spite that it dosn't contain the activity label, which is left to be predicted.

## Preparation

First, the data files are assumed to be downloaded and stored /data/ subdirectory of the working directory.
...
Note: In actual cases, the testing data is not unknown and the nearZeroVar varibales should not be removed.
...
Addinitionally, I have removed several variables ("pitch_belt") which make the fit fail.
...
The number of variables are now 52 from 160.  One of them "classe" is for the target and the variables to be used for training are 63 varibales.
...
However the following train function dos not work well.
